...
private:
    std::vector <unsigned int> mass;
    int sign;
    friend void my_resize(big_integer &a);
    friend int abs_compare(std::vector<unsigned> const& a, std::vector<unsigned> const& b);
    ...
    friend void multiply(std::vector<unsigned> &res, const std::vector<unsigned> &a, uint64_t b);
